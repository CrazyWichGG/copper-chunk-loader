...
execute as @n[type=armor_stand,nbt={Tags:["chunk_loader_animation"]}] run tag @s add chunk_loader_tier.iron

playsound block.anvil.use block @a ~ ~ ~ 1 1

execute as @s[gamemode=!creative] run clear @s *[custom_data~{chunk_loader_upgrade:1b,chunk_loader_upgrade_tier:"iron"}] 1
