execute as @e[type=marker,distance=..1,limit=1,sort=nearest,nbt={data:{chunk_loader_marker:1b}}] run data modify entity @s data.chunk_loader_tier set value "gold"
execute as @e[type=armor_stand,limit=1,sort=nearest,nbt={data:{chunk_loader_animation:1b}}] run data modify entity @s data.chunk_loader_tier set value "gold"
