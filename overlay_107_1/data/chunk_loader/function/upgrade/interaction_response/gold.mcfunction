...
execute as @n[type=armor_stand,nbt={data:{chunk_loader_animation:1b}}] run data modify entity @s data.chunk_loader_tier set value "gold"

playsound block.anvil.use block @a ~ ~ ~ 1 1

execute as @s[gamemode=!creative] run clear @s *[custom_data~{chunk_loader_upgrade:1b,chunk_loader_upgrade_tier:"gold"}] 1
